...
from src.model.mlic import MLIC, MLICDynamicCameraVideoStream
...
class RTIObjectViewer:
    ...
    def __init__(self, 
        ...
        if bi_collection.out_shape != mlic.size:
            raise ValueError(f'Invalid collection shape {bi_collection.out_shape} for MLIC of size {mlic.size}. ')
    ...
    def draw_arrow(self) -> Tuple[Frame, Frame]:
        ...
        cx, cy = self.coord
        light_direction_frame = MLICDynamicCameraVideoStream.draw_line_direction(light_direction=(cx, cy), frame_side=self._frame_size)
